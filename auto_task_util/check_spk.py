...
#     prompt = f"""
# 【分析任务】
# 请根据提供的上下文和句子，确定句子的说话者角色、性别和年龄。任务目标是尽可能准确地识别说话者，优先考虑【角色列表】中的角色。如果无法确定，则使用“未知”。
...
# 【分析步骤】
# 请按照以下**严格**的步骤进行分析，并在每一步中**明确记录**你的思考过程。如果跳过任何步骤，将视为任务失败。
...
# 2. **验证**
#    - 检查初步识别的角色在上下文中是否合理。
#    - 如果发现不一致，记录下不一致的原因，并准备进行综合判断。

# 3. **综合判断**
#    - 如果初步识别和验证结果不一致，分析不一致的原因。
#    - 评估其他可能的角色，并确定最可能的说话者。

# 4. **性别和年龄判定**
#    - 根据上下文中的描述或对话内容，推断说话者的性别和年龄。
#    - 如果没有明确的线索或无法确定，标记为“未知”。

# 【要求】
# - **严格**按照上述步骤输出思考过程。
# - 最终输出为JSON格式，包含role、gender和age字段。
# - gender：仅：男，女，未知。
# - age 仅：未知，青年，中年，老年。
# - 如果无法确定某个字段，请使用“未知”。
# - 输出结果中，请确保字段的顺序为：role、gender和age
...
def process_sentence(
    ...
    try:
        ...
        response = post_request_with_retries(prompt)
        if not response:
            return result
        # logger.info(f"prompt: {prompt}")
        # logger.info(f"response: {response}")
        json_data = extract_json_from_response(response)
        role = json_data.get("role", "未知")
        gender = json_data.get("gender", "未知")
        age = json_data.get("age", "未知")
        logger.info(f"处理句子: {sentence} - 角色: {role}, 性别: {gender}, 年龄: {age}")
...
def process_file(
    ...
    for item in top_roles:
        item_py = generate_pinyin(item)
        if item_py in seen_pinyins:
            continue
        seen_pinyins.add(item_py)
