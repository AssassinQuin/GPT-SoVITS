    ...
    def infer_panel_batch_infer(
        ...
        **kwargs,
        ...
        for x_item, bert_item in zip(x, bert_feature):
            ...
            x_item = (
                F.pad(x_item, (0, 0, 0, max_len - x_item.shape[0]), value=0)
        ...
        for idx in tqdm(range(1500)):
            if idx == 0:
                ...
                xy_dec, k_cache, v_cache = self.t2s_transformer.decode_next_token(
                    xy_pos, k_cache, v_cache, xy_attn_mask, False
                )
            logits = self.ar_predict_layer(xy_dec[:, -1])

            if idx == 0:
                xy_attn_mask = F.pad(
                    xy_attn_mask[:, :, -1].unsqueeze(-2), (0, 1), value=False
                )
                logits = logits[:, :-1]
            else:
            ...
            if reserved_idx_of_batch_for_y is not None:
                ...
                xy_attn_mask = torch.index_select(
                    xy_attn_mask, dim=0, index=reserved_idx_of_batch_for_y
                )
