...
class AudioProcessor:
    ...
    def process_text_line(
        ...
        """
        处理单行文本，包括生成音频、音频质量检测和重试机制，并生成字幕。
    ...
    def process_chapter(self, book_name: str, chapter_index: int) -> None:
        ...
        try:
            ...
            # 保存最终音频文件
            formatted_chapter_index = f"{chapter_index:04d}"
    ...
    def run(self):
        ...
        for task in task_list:
            ...
            # 只有当 gen_tts_end_idx 未设置时才更新
            if "gen_tts_end_idx" not in task or task.get("gen_tts_end_idx") is None:
                task["gen_tts_end_idx"] = total_chapters
            self.save_task_list(task_list)

            if gen_tts_start_idx > task["gen_tts_end_idx"]:
                logger.info(f"书籍 {book_name} 所有章节已处理完毕。")
                task["gen_tts_status"] = "已完成"
                self.save_task_list(task_list)
                continue

            for chapter_idx in range(gen_tts_start_idx, task["gen_tts_end_idx"] + 1):
