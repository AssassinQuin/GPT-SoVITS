...
def update_status(
    ...
    new_entry = {"book_name": book_name, "check_spk_status": status}
    if start_idx is not None:
        new_entry["check_spk_start_idx"] = start_idx
    if top_n is not None:
...
def main():
    ...
    for book in check_spk_list:
        if "check_spk_status" not in book:
            logger.info(
                f"书籍 '{book.get('book_name', '未知')}' 缺少 'status' 字段，设置为 '等待中'"
            )
            book["check_spk_status"] = "等待中"
    ...
    for book in check_spk_list:
        ...
        if not book_name:
            ...
            continue
        status = book.get("check_spk_status", "等待中")  # 确保有默认值
        start_idx = book.get("check_spk_start_idx", DEFAULT_START_IDX)
