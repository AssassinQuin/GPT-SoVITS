...
# # 定义三个不同的模型名称
# MODEL_NAMES = {
#     "model1": "shareai/llama3.1-dpo-zh:latest",  # 请根据实际情况替换为正确的模型名称
#     "model2": "qwen2.5:latest",
#     "model3": "wangshenzhi/gemma2-9b-chinese-chat:latest",
# }


# def post_request_with_retries(
#     prompt: str, model: str, max_retries: int = 3, timeout: int = 10
# ) -> Any:
#     """发送POST请求，并在失败时重试。"""
#     payload = {
#         "model": model,
...
# def construct_model_prompt(sentence: str, context: str, top_roles: str) -> str:
#     """构造用于模型1和模型2的提示语（Prompt）。"""
#     prompt = f"""
# 【原文】：
# {context}
...
# 【已识别的角色】：
# - 模型1判断的角色名称为【{role1.get('role', '未知')}】，性别为【{role1.get('gender', '未知')}】。
# - 模型2判断的角色名称为【{role2.get('role', '未知')}】，性别为【{role2.get('gender', '未知')}】。

# 【任务描述】：
# 请根据原文和句子的上下文，综合模型1和模型2的判断结果，确定该句子的准确说话者并确认其性别。请严格按照以下步骤执行：

# ### 步骤 1：综合评估
# 1.1 比较模型1和模型2给出的角色名称和性别，判断哪一个更符合上下文。

# 1.2 如果两个模型给出相同的角色名称，请确认并返回以下 JSON 格式：
# {{"role": "{role1.get('role', '未知')}", "gender": "{role1.get('gender', '未知')}"}}

# 1.3 如果两个模型给出了不同的角色名称，请根据上下文确定哪个角色更符合当前句子的说话者。如果无法确定，请返回以下 JSON 格式：
# {{"role": "未知", "gender": "未知"}}
...
# ### 步骤 3：性别确认
# 3.1 根据上下文中对角色的描述，确认角色的性别。性别结果必须为 "男"、"女" 或 "未知"。

# 3.2 如果无法从上下文中推断出性别，请返回以下 JSON 格式：
# {{"role": "最终角色姓名", "gender": "未知"}}
...
#     try:
#         # 第一次请求 - 使用模型1
#         prompt1 = construct_model_prompt(sentence, context, top_roles)
#         response_content1 = post_request_with_retries(prompt1, MODEL_NAMES["model1"])
#         role1 = (
#             response_content1
#             if response_content1
#             else {"role": "未知", "gender": "未知"}
#         )

#         # 第二次请求 - 使用模型2
#         prompt2 = construct_model_prompt(sentence, context, top_roles)
#         response_content2 = post_request_with_retries(prompt2, MODEL_NAMES["model2"])
#         role2 = (
#             response_content2
...
#         # 第三次请求 - 使用模型3，基于模型1和模型2的输出
#         prompt3 = construct_summary_prompt(context, sentence, role1, role2, top_roles)
#         response_content3 = post_request_with_retries(prompt3, MODEL_NAMES["model3"])
#         final_role = (
#             response_content3
...
#         # 记录最终结果
#         result[clear_text(sentence, True)] = {
#             "role": final_role.get("role", "未知"),
#             "gender": final_role.get("gender", "未知"),
#             "content": context,
#         }

#         # 更新角色数据
#         if final_role.get("role", "未知") != "未知":
#             role_key = final_role["role"]
#             book_data = {}  # 需要将其传递或管理为全局变量或通过其他方式获取
#             book_data[role_key] = book_data.get(
#                 role_key, {"count": 0, "last_chapter": 0}
#             )
#             book_data[role_key]["count"] += 1
#             # last_chapter 在调用处设置
#         # logger.info(
#         #     f"处理句子: {sentence}, 角色: {final_role.get('role', '未知')}, 性别: {final_role.get('gender', '未知')}"
#         # )
